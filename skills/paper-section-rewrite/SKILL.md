...
description: 학술 논문·연구계획서 한 섹션을 LLM이 자율적으로 4-Level critic loop로 분석·개선하는 윤문 스킬. 설계 순서를 강제 — Level 1 내용 → Level 2 문단 간 구성 → Level 3 문단 내 구성 → Level 4 문장. Level 1·2·3는 CONVERGED skip 금지. 두괄식 + 의도(질문)→방법→결과→해석. paper-digest의 insight-first(단순 나열 X), iterative-academic-writing의 논리적 비약 0, 남는 그림(단위마다 읽고 나면 남는 한 줄 — 논문·섹션·소절·문단에 똑같이 적용, 위 단위의 그림은 아래 단위 그림들의 합), 세 층의 유기성(섹션 사이·문단 사이·문장 사이), 이력 독립성(이 글만 읽는 독자가 모르는 경과 표현 0), 긍정형 우선(부정 결과는 범위를 담은 긍정문으로), humanize의 자연스러움(em-dash 0, 번역투 0, 계사 대용 관용구 "~에 해당한다/~로 기능한다/~에 위치한다/자리매김" 0, 동족어·중복어 stacking 0, "~하고자 한다" 비율 ≤ 30%, register 일관성, 개념어 과반복 회전, 종결어미 다양성). 한 문장 한 역할(One Sentence One Role) + 단문 비율 ≥ 60% 강제. ML 비전공자 가독성 게이트(약어 풀어쓰기 / 수식 전 자연어 / jargon ≤ 3 / 서술형 풀어쓰기 — 명사형 종결 금지). 섹션 자기 완결성. 본문+부록 전체 수식·notation 감사 (중복/혼용/오용/Orphan 검출). KO/EN + 연구계획서 장르 지원. MANDATORY TRIGGERS - 섹션 다시 쓰기, 섹션 윤문, 풀어쓰기, 두괄식, paper section rewrite, 연구계획서 윤문, 자연스럽게, AI 티 빼줘, ML-beginner, notation audit, em-dash 0, 단문, 단문 비율, 한 문장 한 역할, insight-first, 자기 완결, 논리적 비약, 서술형 풀어쓰기, 명사형 종결 금지, 계사 대용, 해당한다 위치한다 기능한다, 동족어 stacking, 하고자 한다 줄이기, register 일관성, 개념어 과반복, 종결어미 다양성, 남는 그림, 유기성, 문단 연결, 건너뜀, 지리멸렬, 워킹노트 제거, 작업 노트, 이력 독립, 경과 표현, 긍정형, 부정형 줄이기
---
...
```
Level 1 [내용]         의도·내용 1문장 + insight-first + 남는 그림 표  →  CONVERGED
Level 2 [문단 간 구성]  단락 outline + R1~R5 + 의도→방법→결과→해석 + lead 합=섹션 그림  →  CONVERGED
Level 3 [문단 내 구성]  단락별 카테고리 흐름 + 권장 흐름 일치  →  CONVERGED
Level 4 [문장]         단문 게이트 + 문장 연결 + 긍정형 우선 + 자연스러움 + 논리적 비약 + ML-beginner  →  CONVERGED
```
...
[Phase 5]   섹션 통합 critic + 자기 완결성 + 이력 독립성 + 남는 그림 역검증
...
**읽고 나면 무엇이 남는가.** 단위마다 독자에게 남길 그림을 한 줄로 적는다. 이 기준은 초록에만
쓰는 것이 아니다. 논문, 섹션, 소절, 문단에 똑같이 적용하며, **위 단위의 그림은 아래 단위
그림들의 합**이어야 한다.

| 단위 | 남는 그림이 놓이는 자리 | 아래 단위와의 관계 |
|---|---|---|
| 논문 | 초록 | 섹션 그림들을 이으면 초록이 된다 |
| 섹션 | 섹션 도입 2~3문장 | 소절의 주장 한 줄들을 이으면 섹션 그림이 된다 |
| 소절 | 굵은 주장 한 줄(Finding) | 문단들이 의도·가설·방법·결과·해석 가운데 하나씩 맡는다 |
| 문단 | 첫 문장(lead) | 나머지 문장은 그 lead를 받치는 역할만 한다 |
| 문장 | 역할 하나 | 앞 문장이 끝낸 말에서 출발한다 |

절차:

1. 손대기 전에 섹션, 소절, 문단마다 남길 그림을 한 줄씩 적은 표를 만든다.
2. 위 단위 한 줄이 아래 단위 한 줄들의 합과 맞는지 본다. 맞지 않으면 문장이 아니라 **구조**
   문제다. Level 2에서 문단 배열을 먼저 고친다.
3. 그림에 기여하지 않는 문단과 문장은 이동(부록) 또는 삭제 후보로 표시한다.

본보기. 다섯 문장 초록이 목적, 접근, 행동 결과, 내부 결과, 의미를 하나씩 맡으면 읽고 난
독자에게 "자율이 위험을 키우고 내부 특성이 그것을 제어한다"는 그림 하나가 남는다. 같은 정보를
여덟 문장에 단서와 양보절을 섞어 담으면 더 정확해도 그림이 남지 않는다
(`feedback-corpus.md` R7-1).
...
| 남는 그림 표 ★ | 단위마다 한 줄이 있고, 위 단위 한 줄 = 아래 단위 한 줄들의 합 |

미달이면 LLM이 두 문장을 자율 재작성 (최대 3 라운드). 5 게이트 모두 통과 = **Level 1 CONVERGED → Level 2 진행**.
...
### Step 2.5 — Insight-first 게이트 (단락 lead)
...
| lead 합 = 섹션 그림 ★ | lead만 이어 읽으면 Step 1.4b의 섹션(소절) 한 줄이 나온다 |
| 단위 사이 연결 ★ | 앞 소절·문단의 끝 문장이 다음 소절·문단의 질문을 연다. 같은 내용은 한 곳에서만 주장하고 다른 곳은 근거나 함의를 맡는다 |
| 소절 모양 반복 ★ | 결과 섹션의 소절은 같은 틀(주장 한 줄 → 설계 → 결과·해석 → 표나 그림 하나)을 되풀이한다 |
...
| 문장 역할 합 = 문단 그림 ★ | 문단을 덮었을 때 lead가 그대로 떠오른다. lead를 받치지 않는 문장 0 |
| 문단당 그림 하나 ★ | 한 문단에 결과가 둘 이상이면 나눈다. 방법 문단은 결과를 이해하는 데 필요한 만큼만 남기고 나머지는 부록 |
| 정의가 사용보다 앞 ★ | 용어는 처음 쓰는 자리에서, 무엇을 하는지로 풀어 정의한다 |
...
**단문 비율 게이트 (Hard)**: 단락 안 단문(KO 50자 / EN 20w 이내) 비율 **≥ 60%** 강제. 미만이면 prose가 무거워지고 ML 비전공자 가독성이 떨어진다. 60% 미만 단락은 substep 자동 fail → 긴 문장을 분할하거나 압축해 비율 통과시킨 뒤 재라벨. 80% 이상은 호평.

하이브리드 라벨 `[정의(+인과)]` **금지** — 무조건 `[혼합:A+B] ✗`로 표기 후 분할.

상세: `writing-principles-ko.md` "단문 원칙 — One Sentence, One Role" 절.

#### Step 4.i.2 — 자연스러움 게이트
...
| **계사 대용 관용구** | "~에 해당한다 / ~에 위치한다 / ~로 기능한다 / ~로 자리매김하다 / ~방향에 있다 / ~핵심에 해당한다 / 입력으로 기능한다" (`banned-phrases-ko.md` §7) | "serves as / functions as / is positioned within / amounts to" 남발 |
| **동족어·중복어 stacking** | "산출물을 산출 / 연구의 연구 / 다시 재구성 / 가능한 도달 가능한 / 동일한 X3" (`banned-phrases-ko.md` §8) | repeated cognate within a sentence |
...
| **Register 불일치** | 갑작스런 문어체("아니한다 / 상기 / 전술한") ↔ 갑작스런 구어체("끌려간 / 흔들림 / 넓게 모은다 / 막고 / 가져온다") (`banned-phrases-ko.md` §9) | archaic ("hereinabove") ↔ casual ("grab / a bunch of") 혼재 |
...
4. **서술형 풀어쓰기 — 명사형 종결 금지**: **정의된 단어·복잡한 jargon을 명사형으로 끝내지 말고 동사로 풀어 설명**한다. "X는 Y이다 (명사형으로 굳혀 끝)" 대신 "X는 ~을 ~한다 (서술형, 어떻게 작동하는지 풀어쓰기)"로.
...
#### Step 4.i.6 — 문장 연결 게이트 ★

문장 표현이 아니라 **문장과 문장 사이**를 본다. 실패 유형은 둘이다.

| 실패 | 모양 | 처방 |
|---|---|---|
| **건너뜀** | 추론 고리가 빠졌다. 예: "방향들은 거의 직교한다. 공유 부분공간이 전이된다." 직교하는데 어떻게 공유되는지 말하지 않는다 | 빠진 고리를 한 문장으로 적는다 ("공통 성분이 작아 코사인에는 안 잡히지만 두 과제를 합치면 분리된다") |
| **지리멸렬** | 같은 말을 되풀이하거나(같은 유보를 세 번), 결과를 이해하는 데 필요 없는 방법 세부가 길다 | 한 번만 말한다. 세부는 부록으로 |

연결 규칙:

1. 다음 문장은 앞 문장이 끝낸 말(신정보)을 주어나 화제로 받아 출발한다.
2. 추론의 고리("because", "so", "yet", "그래서", "반면")를 글로 적는다. 접속어는 논리가
   꺾이는 자리에만 둔다.
3. 나란한 결과는 같은 문형으로 쓴다 (모델 A와 모델 B, 더하기와 지우기).
4. 문단의 마지막 문장은 그 결과의 의미를 말하고, 그 의미가 다음 문단·소절의 질문이 된다.
5. **원인과 결과처럼 한 역할인 쌍은 한 문장으로 합친다.** 단문 비율은 60%가 하한이고
   **80%를 넘으면 ⚠ "개조식 위험"**이다. 접속 없이 짧은 문장이 네 개 이상 이어지면 ✗.
   (예: "편집은 밴드에 가한다. Gemma는 L16–21이다. 단일 층은 움직이지 않았기 때문이다."
   → "한 층만 고쳐서는 베팅이 움직이지 않았으므로 연속한 여섯 층(L16–21)을 함께 편집한다.")

판정: 문단당 건너뜀 ≥ 1 또는 지리멸렬 ≥ 1 = ✗.

#### Step 4.i.7 — 긍정형 우선 게이트 ★

부정으로 보이는 문장은 세 종류다. 섞어 다루지 않는다.

| 종류 | 뜻 | 처리 |
|---|---|---|
| **부정 결과** | 주장과 어긋나거나 주장을 다른 조건으로 넓히지 못한 결과 | 본문에는 **범위를 담은 긍정문**만 남긴다 ("Gemma에서 목표 모듈은 기울기를 가파르게 한다"). "LLaMA에서는 재현되지 않는다"는 부록에 둔다 |
| **주장을 받치는 대비** | null이지만 주장의 일부인 결과 ("판독 방향은 판돈을 움직이지 못한다"가 "읽기와 쓰기는 다르다"를 받친다) | 남긴다. 긍정형으로 쓴다 ("판독 방향은 상태를 보고하고, 판돈은 행동 유래 방향이 정한다") |
| **문법만 부정** | 내용은 긍정인데 부정문 ("~은 설명하지 못한다", "does not claim", "Not X:") | 긍정형으로 바꾼다 |

임계: 소절 주장 한 줄과 문단 lead의 부정형 **0**. 문단당 부정형 ≤ 1 (대비 null 제외).
검출 패턴은 `banned-phrases-en.md` §13, `banned-phrases-ko.md` §11.

**안전장치 (Hard).** 부정 결과를 긍정문으로 바꿀 때 (1) 범위 한정어("Gemma에서", "세 API
모델에서", "L22에서")를 반드시 남기고, (2) 부록의 해당 결과와 모순이 없는지 대조한다. 범위를
지우면 이 게이트가 과장을 만든다. 유보 표현("weak", "rather than accurate predictors")을 지울
때도 같다. 평가어를 지우는 대신 수치와 통과한 통제를 그대로 적는다. 주장이 어디까지인지는
숫자가 말한다.
...
| 계사 대용 | "해당한다 / 위치한다 / 기능한다 / 자리매김 / 방향에 있다" 단정문으로 — 단락당 0 (`banned-phrases-ko.md` §7) |
| 동족어 stacking | "산출물을 산출 / 연구의 연구 / 다시 재구성" 류 문장당 0 (§8) |
| register 일관성 | 갑작스런 문어체("아니한다")·구어체("끌려간/흔들림") 0 (§9) |
| 논리적 비약 | 단락당 비약 0 |
| ML-beginner | lead·본문 4개 항목 통과 (약어 풀어쓰기 / 수식 전 자연어 / jargon ≤ 3 / **서술형 풀어쓰기 — 명사형 종결 X**) |
| 정량 grep | 모든 임계 통과 (것이다·에 대한·계사 대용·"~하고자 한다" 비율 ≤ 0.3 포함) |
| 문장 연결 ★ | 문단당 건너뜀 0, 지리멸렬 0. 단문 비율 60~80%, 접속 없는 단문 4연속 0 |
| 긍정형 우선 ★ | 주장 한 줄·lead의 부정형 0, 문단당 부정형 ≤ 1. 범위 한정어 유지 + 부록과 모순 0 |
| 4축 통합 | 9/12 이상 |
...
### Step 5.2b — 이력 독립성 게이트 ★

**이 글만 읽는 독자가 이해할 수 없는 표현은 0.** 자기 완결성 게이트가 다른 섹션에 기대는 것을
잡는다면, 이 게이트는 **이전 버전과 작업 과정에 기대는 것**을 잡는다. 본문, 캡션, 부록 모두에
적용한다.

| 유형 | 영어 시그널 | 한국어 시그널 |
|---|---|---|
| 시간 표지 | earlier, previously, now, no longer, still, originally, at the time | 이전, 앞서의, 이제, 더 이상, 아직, 원래, 당시 |
| 판본·작업 이름 | corrected design, legacy runs, new/old version, main study, follow-up | 교정된 설계, 레거시, 구버전, 메인 연구, 후속 |
| 과정 서술 | re-run, re-analysis, re-audit, we then/later, additionally ran | 다시 돌린, 재분석, 재감사, 추가로 돌린 |
| 편집 결정 서술 | the body keeps…, we moved…, is now reported in…, kept for transparency | 본문은 ~를 유지했다, ~로 옮겼다, 투명성을 위해 남긴다 |
| 심사 경과 | in this revision, as requested, reviewers, rebuttal, camera-ready | 수정본에서, 요청에 따라, 리뷰어, 리버탈 |

처방은 하나다. **시간으로 부르지 말고 내용으로 부른다.** 구분이 실제 정보를 담고 있으면
구분은 남기고 이름만 바꾼다.

- "the corrected design" → "the matched-loss design" (세 선택지의 기대 손실이 같은 설계)
- "legacy runs" → "the closed-model runs, whose mid-variance option loses slightly more"
- "three earlier single-layer protocols" → "three single-layer protocols at L22"
- "main study / follow-up" → 실험의 역할이나 과제 이름
- "re-analysis" → 분석의 이름 ("a cluster-robust analysis")

**주의.** 작업 이름이 설계 이름으로 굳는 경우가 가장 늦게 발견된다. 원고를 고치며 붙인
"corrected"가 본문에서 실험 설계의 이름이 되면, 독자는 "무엇을 왜 고쳤나, 고치기 전 결과는
틀린 것인가"를 묻게 된다. 본문을 단순히 가리키는 참조("Table 3 of the body")는 위반이 아니다.
통계 용어("FDR-corrected", "Bonferroni-corrected")도 위반이 아니다.

판정: 위 유형 **1건이라도** 남으면 ✗. 패턴은 `banned-phrases-en.md` §12, `banned-phrases-ko.md` §10.

### Step 5.3 — 섹션 간 중복 표현 게이트

같은 표현·문장이 다른 섹션에서 반복되는지 검출 (입력에 다른 섹션이 함께 제공된 경우).

처리: 반복 발견 시 **패러프레이징 강제**. 한 섹션은 정의, 다른 섹션은 활용 시점 등 역할 분담.

### Step 5.4 — 캡션↔본문 중복 게이트
...
Step 1.4b의 표를 아래에서 위로 다시 확인한다.

1. **lead 이어 읽기.** 문단의 첫 문장만 뽑아 이어 읽는다. 소절의 주장 한 줄과 같은 말을 해야
   한다. 소절의 주장 한 줄만 이어 읽으면 섹션 그림이, 섹션 그림을 이으면 초록이 되어야 한다.
   초록은 이 과정의 끝에 나오는 결과이지 따로 쓰는 글이 아니다.
2. **처음 읽는 독자 검증.** 이 글만 읽은 독자(별도 critic)에게 단위를 읽힌 뒤 남은 그림을 한
   줄로 쓰게 한다. 미리 적어 둔 한 줄과 뜻이 다르면 ✗. 이 독자에게는 이전 버전, 작업 기록,
   저자의 의도 메모를 주지 않는다.

판정: 어느 단위든 역검증 한 줄이 Step 1.4b의 한 줄과 어긋나면 그 단위를 Level 2로 되돌린다.
