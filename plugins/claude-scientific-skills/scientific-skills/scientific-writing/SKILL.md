...
### 12. CRITICAL: Hallucination Prevention (사실 검증)

**⚠️ Scientific writing에서 Hallucination은 절대 금지된다.**

학술 글쓰기의 신뢰성은 모든 주장이 검증 가능한 사실에 기반해야 한다. 검증되지 않은 정보, 존재하지 않는 인용, 잘못된 수치는 학술적 무결성을 심각하게 훼손한다.

#### 12.1 Hallucination 유형 및 검증 방법

| Hallucination 유형 | 설명 | 검증 방법 |
|-------------------|------|----------|
| **허위 인용 (Fake Citation)** | 존재하지 않는 논문 인용 | WebSearch로 논문 존재 여부 확인 |
| **잘못된 저자/연도** | 실제와 다른 저자명 또는 발행 연도 | 원본 논문 메타데이터 확인 |
| **잘못된 통계/수치** | 원본과 다른 숫자 인용 | 원본 데이터/논문에서 직접 확인 |
| **과장된 주장** | 결과를 과대 해석 | 실험 데이터와 주장 비교 |
| **존재하지 않는 방법론** | 실제로 사용하지 않은 방법 서술 | 실험 기록/코드와 대조 |
| **허위 결과** | 실험하지 않은 결과 보고 | 원본 데이터 파일 확인 |

#### 12.2 인용 검증 프로세스 (Mandatory)
...
인용하려는 논문:
│
├─ [Step 1] WebSearch로 논문 존재 여부 확인
│   "Author et al. YYYY Title Journal"
│
├─ [Step 2] 메타데이터 검증
│   - 저자명 정확성
│   - 발행 연도 정확성
...
실험 결과나 통계 수치 작성 시:
- [ ] 원본 데이터 파일에서 수치를 직접 확인했는가?
- [ ] 통계 분석 결과 파일과 일치하는가?
- [ ] 유효 숫자/소수점이 원본과 동일한가?
- [ ] 단위가 정확한가?
- [ ] p-value, confidence interval이 정확한가?

#### 12.4 절대 금지 사항

**❌ 절대 하지 말 것:**
- 읽지 않은 논문 인용
- 기억에 의존한 수치 작성 (반드시 원본 확인)
- 존재 여부 확인 없이 논문 인용
- 추측을 사실처럼 서술
- 원본 확인 없이 2차 출처에서 인용

**✅ 반드시 할 것:**
- 인용 전 WebSearch로 논문 존재 확인
- 모든 수치는 원본 데이터에서 복사
- 불확실한 내용은 "~로 보고되었다" 등 조건부 서술
- 검증 불가능한 내용은 제외
...
검증할 수 없는 정보는:
1. **제외**: 해당 내용을 원고에서 삭제
2. **조건부 서술**: "~로 알려져 있으나 원본 확인 필요"
3. **대체 인용**: 검증 가능한 다른 출처 사용
4. **명시적 표기**: 검증 대기 상태임을 명시 (내부 검토용)
